...
To set Chaturbate credentials, a `PASSWORD`, `AUTO_RECORD_USERS`, or an outbound proxy (`PSTREAMREC_PROXY_URL`, `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`), edit the `environment:` section of `docker-compose.yml` in the installed app directory on your Umbrel (`~/umbrel/app-data/p-streamrec-store-p-streamrec/`) and restart the app from the UI.
